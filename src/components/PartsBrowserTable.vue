...
export default {
  ...
  methods: {
    ...
    selectPart(part) {
      console.log('Selected part from table:', part);
      ...
      try {
        ...
        const partProperties = {
          ...
          haystackTag: `{id:${part.ItemNumber || 'unknown'}, ${(part.Class || 'part').toLowerCase()}:true}`,
        ...
        console.log('Formatted part properties:', partProperties);
        
        // Create a combined object with all properties
        const combinedProperties = {
          ...partProperties,
          _originalPart: originalPart
        };
        
        // Log the combined properties for debugging
        console.log('Combined part properties being emitted:', JSON.stringify(combinedProperties, null, 2));
        
        // Emit the combined properties
        this.$emit('select', combinedProperties);
      } catch (error) {
        ...
        this.$emit('select', {
          ...
          manufacturer: part.Manufacturer || '',
          quantity: 1,
          _originalPart: part // Include the original part even in error case
        });
      }
    }
  }
