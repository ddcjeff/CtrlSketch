...
export default {
  ...
  data() {
    return {
      ...
      _renderDebounceTimeout: null,
      _renderDebounceDelay: 16, // ms (roughly one frame at 60fps)
  ...
  watch: {
    selectedShapes: {
      immediate: true,
      handler(newVal) {
        console.log('selectedShapes prop changed:', newVal ? newVal.length : 0);
        
        // Skip processing if newVal is undefined or null
        if (!newVal) return;
        
        // Create arrays of IDs for comparison
        const currentIds = this.localSelectedShapes.map(s => s.id).sort();
        const newIds = newVal.map(s => s.id).sort();
        
        // Check if arrays are different by comparing stringified versions
        const currentStr = JSON.stringify(currentIds);
        const newStr = JSON.stringify(newIds);
        
        if (currentStr !== newStr) {
          console.log('Selection changed, updating local selection');
          
          // Use setTimeout to break the reactivity chain
          setTimeout(() => {
            // Break reactivity chain with deep clone
            if (newVal.length > 0) {
              this.localSelectedShapes = JSON.parse(JSON.stringify(newVal));
            } else {
              this.localSelectedShapes = [];
            }
            
            // Only render if we're not in the middle of another operation
    ...
    shapes: {
      ...
      handler(newVal, oldVal) {
        ...
        if (Array.isArray(newVal)) {
          newVal.forEach(shape => {
            if (shape && shape.type === 'image') {
              if (shape.src && !shape.image && !shape._isLoading) {
                // Queue the image for loading instead of loading immediately
        ...
        if (oldVal && Array.isArray(oldVal) && Array.isArray(newVal)) {
          ...
          newVal.forEach((shape) => {
            ...
            if (shape.type === 'image' && oldShape) {
              ...
              if (oldShape.layerId !== shape.layerId && !oldShape.image && !shape._isLoading) {
                // Queue the image for loading instead of loading immediately
                if (!imagesToLoad.some(img => img.id === shape.id)) {
                  imagesToLoad.push({...shape}); // Clone to avoid modifying the original
                }
              }
            }
  ...
  methods: {
    ...
    arraysEqual(arr1, arr2) {
      ...
      // Compare stringified versions for more reliable comparison
    ...
    reloadImage(shape, showErrorNotification = true) {
      // Safety check for valid shape
      if (!shape || !shape.id) {
        console.error('Invalid shape passed to reloadImage');
        return;
      }
      ...
      const shapeClone = {...shape, _isLoading: true};
      ...
      if (!shapeClone.src) {
        if (showErrorNotification) {
          this.$emit('show-notification', {
            ...
            details: `Shape ID: ${shapeClone.id}`,
            duration: 5000
          });
        }
        ...
        const updatedShapes = this.shapes.map(s => 
          s && s.id === shapeClone.id ? { ...s, imageError: 'missing-source', _isLoading: false } : s
        );
      ...
      if (this.imageCache.has(shapeClone.id)) {
        // Don't modify the original shape directly
        const cachedImage = this.imageCache.get(shapeClone.id);
        
        // Update the shape in the shapes array and clear any previous errors
        // Also ensure stroke and fill are null for image shapes
        const updatedShapes = this.shapes.map(s => 
          s && s.id === shapeClone.id ? { 
            ...s, 
            image: cachedImage, 
            imageError: null,
            stroke: null,
      ...
      const loadingShapes = this.shapes.map(s => 
        s && s.id === shapeClone.id ? { ...s, isLoading: true, _isLoading: true, imageError: null } : s
      );
      ...
      const handleMaxRetriesReached = (reason) => {
        ...
        const errorShapes = this.shapes.map(s => 
          s && s.id === shapeClone.id ? { ...s, isLoading: false, _isLoading: false, imageError: reason } : s
        );
              ...
              actions: [
                {
                  ...
                  callback: () => {
                    // Use setTimeout to break the reactivity chain
                    setTimeout(() => {
                      const freshShape = this.shapes.find(s => s && s.id === shapeClone.id);
                      if (freshShape) {
                        this.reloadImage({...freshShape, _isLoading: false}, true);
                      }
                    }, 50);
                  }
                },
                {
                  label: 'Edit URL',
                  callback: () => {
                    const newUrl = prompt('Enter new image URL:', shapeClone.src);
                    if (newUrl && newUrl !== shapeClone.src) {
                      const updatedShapes = this.shapes.map(s => 
                        s && s.id === shapeClone.id ? { ...s, src: newUrl, imageError: null, _isLoading: false } : s
                      );
                      ...
                      setTimeout(() => {
                        ...
                        // Schedule reload with a longer delay
                        setTimeout(() => {
                          const freshShape = this.shapes.find(s => s && s.id === shapeClone.id);
                          if (freshShape) {
                            this.reloadImage({...freshShape, src: newUrl, _isLoading: false}, true);
                          }
                        }, 100);
                      }, 0);
                    }
      ...
      const loadImage = () => {
        ...
        img.onload = () => {
          ...
          this.imageCache.set(shapeClone.id, img);
            ...
            if (file) {
              ...
              reader.onload = (e) => {
                try {
                  ...
                  // Update the shape with the Excel data
                  const updatedShape = {
                    ...shape,
                    excelData: {
                      workbook: workbook,
    ...
    _hexToRgb(hex) {
      try {
        ...
        console.error('Error converting hex to RGB:', error, hex);
    ...
    render() {
      ...
      // Set flag to prevent multiple render requests
      ...
      // Use a more robust debouncing approach with a weak reference to this
      const self = this;
      
      // Schedule a new render with debouncing
      this._renderDebounceTimeout = setTimeout(function() {
        ...
        self.animationFrameId = requestAnimationFrame(function() {
          try {
            ...
            self._renderDebounceTimeout = null;
            
            if (!ctx) return;
            
            // Capture all values locally to prevent reactive dependencies
            const currentGridSize = self.gridSize;
            const currentGridOpacity = self.gridOpacity;
            const currentGridType = self.gridType;
            const currentGridColor = self.gridColor;
            const currentCanvasWidth = self.canvasWidth;
            const currentCanvasHeight = self.canvasHeight;
            const currentZoom = self.zoom;
            
            // Check if static canvas needs to be updated
            if (self.staticCanvasDirty || 
                self.lastGridSize !== currentGridSize || 
                self.lastGridOpacity !== currentGridOpacity ||
                self.lastGridType !== currentGridType ||
                self.lastGridColor !== currentGridColor ||
                self.lastCanvasWidth !== currentCanvasWidth ||
                self.lastCanvasHeight !== currentCanvasHeight ||
                self.lastZoom !== currentZoom) {
              
              // Update static canvas
              self._renderStaticElements();
              ...
              self.lastCanvasHeight = currentCanvasHeight;
              self.lastZoom = currentZoom;
              self.staticCanvasDirty = false;
            }
            
            // Perform the main render
            self._performRender(ctx);
            ...
            if (renderTime > 50) {
              ...
              self.performanceMetrics.slowRenders++;
              self.performanceMetrics.peakRenderTime = Math.max(self.performanceMetrics.peakRenderTime, renderTime);
            }
            
            // Update performance metrics
            self.performanceMetrics.lastRenderTime = renderTime;
            self.performanceMetrics.renderCount++;
            
            // Calculate average render time
            const alpha = 0.1; // Weight for new value in exponential moving average
            self.performanceMetrics.averageRenderTime = 
              alpha * renderTime + (1 - alpha) * self.performanceMetrics.averageRenderTime;
            
          } catch (error) {
            console.error('Error during render:', error);
            // Reset flags to allow future renders
            self._renderRequested = false;
            self._renderDebounceTimeout = null;
          }
        });
      }, this._renderDebounceDelay);
    },
    ...
    _renderStaticElements() {
      ...
      // Capture all values locally to prevent reactive dependencies during rendering
      ...
      const currentGridOpacity = this.gridOpacity || 0.5;
      const currentGridType = this.gridType || 'square';
      const currentGridColor = this.gridColor || '#000000';
      
      // Resize static canvas if needed
      if (this.staticCanvas.width !== canvasWidth || this.staticCanvas.height !== canvasHeight) {
      ...
      const centerX = (canvasWidth - canvasWidth * currentZoom) / 2 + currentOffsetX;
      const centerY = (canvasHeight - canvasHeight * currentZoom) / 2 + currentOffsetY;
      ...
      if (currentGridSize) {
        ...
        const opacity = currentGridOpacity;
        const rgbColor = this._hexToRgb(currentGridColor);
        ...
        // Removed console.log to prevent recursive updates
        ...
        ctx.save();
          ...
          if (type === 'rectangle') {
            ...
            if (shape.image instanceof HTMLImageElement && shape.image.complete) {
              try {
                // Only draw a border if the shape is selected or if explicitly requested
                if (this.localSelectedShapes.some(s => s.id === shape.id) || shape.showBorder) {
            ...
            // No image yet, try to load it
              ...
              if (shape.src && !shape._isLoading) {
                // Set a flag to prevent multiple loading attempts
                shape._isLoading = true;
                
                // Queue image loading for after rendering to prevent recursive updates
                setTimeout(() => {
                  // Get a fresh reference to the shape
                  const freshShape = this.shapes.find(s => s && s.id === shape.id);
                  if (freshShape && freshShape.src) {
                    this.reloadImage({...freshShape}, false); // Don't show notification on auto-reload
                  }
                }, 200);
              }
            }
            ...
            if (shape.excelData && shape.excelData.tableData && shape.excelData.tableData.length > 0) {
              ctx.save();
              ...
              const headers = shape.excelData.headers || [];
              const numColumns = headers.length;
              const colWidth = numColumns > 0 ? shape.width / numColumns : shape.width;
              
              // Draw table header
              ctx.fillStyle = '#f3f4f6'; // Light gray background for header
              ctx.fillRect(shape.x, shape.y, shape.width, headerHeight);
              ...
              headers.forEach((header, index) => {
                ctx.fillText(
                  String(header).substring(0, Math.floor(colWidth / 8)), // Limit text length
                  shape.x + (index * colWidth) + padding,
                  shape.y + headerHeight / 2
                );
              });
              
              // Draw header bottom border
              ctx.beginPath();
              ctx.moveTo(shape.x, shape.y + headerHeight);
              ctx.lineTo(shape.x + shape.width, shape.y + headerHeight);
              ...
              const availableHeight = shape.height - headerHeight;
              const maxRows = Math.floor(availableHeight / rowHeight);
              const rowsToDisplay = Math.min(maxRows, shape.excelData.tableData.length);
              
              // Draw each visible row
              for (let i = 0; i < rowsToDisplay; i++) {
                const row = shape.excelData.tableData[i];
                const y = shape.y + headerHeight + (i * rowHeight);
                
                // Draw row background (alternating colors)
                ctx.fillStyle = i % 2 === 0 ? '#ffffff' : '#f9fafb';
                ctx.fillRect(shape.x, y, shape.width, rowHeight);
                ...
                row.forEach((cell, j) => {
                  if (j < numColumns) {
                    ctx.fillText(
                      String(cell).substring(0, Math.floor(colWidth / 8)), // Limit text length
                      shape.x + (j * colWidth) + padding,
                      y + rowHeight / 2
                    );
                  }
                });
                ...
                ctx.beginPath();
                ctx.moveTo(shape.x, y + rowHeight);
                ctx.lineTo(shape.x + shape.width, y + rowHeight);
              ...
              // Draw column dividers
               for (let i = 1; i < numColumns; i++) {
                ctx.beginPath();
                ctx.moveTo(shape.x + (i * colWidth), shape.y);
                ctx.lineTo(shape.x + (i * colWidth), shape.y + headerHeight + (rowsToDisplay * rowHeight));
                ctx.strokeStyle = '#e5e7eb';
                ctx.lineWidth = 0.5 / this.zoom;
                ctx.stroke();
              }
              ...
              // Draw a grid pattern to indicate it's a table
              ctx.strokeStyle = '#d1d5db';
              ctx.lineWidth = 0.5 / this.zoom;
              
              // Draw horizontal lines
              const rowHeight = shape.height / 5;
              for (let i = 1; i < 5; i++) {
                ctx.beginPath();
                ctx.moveTo(shape.x, shape.y + i * rowHeight);
                ctx.lineTo(shape.x + shape.width, shape.y + i * rowHeight);
                ctx.stroke();
              }
              
              // Draw vertical lines
              const colWidth = shape.width / 4;
              for (let i = 1; i < 4; i++) {
                ctx.beginPath();
                ctx.moveTo(shape.x + i * colWidth, shape.y);
                ctx.lineTo(shape.x + i * colWidth, shape.y + shape.height);
                ctx.stroke();
              }
              ...
              ctx.textAlign = 'center';
              ctx.textBaseline = 'middle';
              ctx.fillText('Double-click to import Excel data', shape.x + shape.width/2, shape.y + shape.height/2);
              ctx.textAlign = 'left';
              ctx.textBaseline = 'alphabetic';
    ...
    animate() {
      // Capture current values to prevent reactive dependencies
      const currentZoomTarget = this.zoomTarget;
      const currentZoom = this.zoom;
      const currentPanTargetX = this.panTargetX;
      const currentPanTargetY = this.panTargetY;
      const currentOffsetX = this.offsetX;
      const currentOffsetY = this.offsetY;
      
      // Calculate differences
      const zoomDiff = currentZoomTarget - currentZoom;
      const offsetXDiff = currentPanTargetX - currentOffsetX;
      const offsetYDiff = currentPanTargetY - currentOffsetY;
      ...
      if (Math.abs(zoomDiff) > 0.0001) {
        newZoom = currentZoom + zoomDiff * 0.15;
      } else {
        newZoom = currentZoomTarget;
      }
      
      if (Math.abs(offsetXDiff) > 0.1) {
        newOffsetX = currentOffsetX + offsetXDiff * 0.2;
      } else {
        newOffsetX = currentPanTargetX;
      }
      
      if (Math.abs(offsetYDiff) > 0.1) {
        newOffsetY = currentOffsetY + offsetYDiff * 0.2;
      } else {
        newOffsetY = currentPanTargetY;
      }
      
      // Update values only if they've changed
    ...
    handleContextMenuAction(action) {
      ...
      switch (action.type) {
        ...
        case 'duplicate':
          ...
          break;
        case 'bringToFront':
          this.$emit('bring-to-front', this.localSelectedShapes.map(s => s.id));
          break;
        case 'sendToBack':
          this.$emit('send-to-back', this.localSelectedShapes.map(s => s.id));
          break;
        case 'makeShapePart':
          this.$emit('make-shape-part', this.localSelectedShapes);
          break;
        case 'groupShapes':
          this.$emit('group-shapes', this.localSelectedShapes);
          break;
        case 'ungroupShapes':
          this.$emit('ungroup-shapes', this.localSelectedShapes);
          break;
        case 'align':
          this.$emit('align-shapes', this.localSelectedShapes);
          break;
        case 'properties':
          this.$emit('show-properties', this.localSelectedShapes[0]);
          break;
      }
    },
    
    /**
     * Handle dragover event for the canvas
     * This is needed to allow dropping
     */
    handleDragOver(event) {
      // Change the cursor to indicate that dropping is allowed
      event.dataTransfer.dropEffect = 'copy';
    },
    
    /**
     * Handle drop event for the canvas
     * This is triggered when a shape is dropped from the library
     */
    handleDrop(event) {
      try {
        // Get the drop position in canvas coordinates
        const rect = this.$refs.canvas.getBoundingClientRect();
        const dropX = (event.clientX - rect.left) / this.zoom - this.offsetX;
        const dropY = (event.clientY - rect.top) / this.zoom - this.offsetY;
        
        console.log('Drop event at canvas coordinates:', { dropX, dropY });
        ...
        // Check if this is a library shape being dropped onto the canvas
        if (parsedData.type === 'library-shape') {
          this.addLibraryShapeToCanvas(parsedData, dropX, dropY);
        }
      } catch (error) {
    ...
    /**
     * Add a shape from the library to the canvas
     */
    addLibraryShapeToCanvas(droppedData, x, y) {
      const { shapeData, partProperties } = droppedData;
      
      // Create a deep copy of the shape data to avoid reference issues
      const newShape = JSON.parse(JSON.stringify({
        ...shapeData,
        id: 'shape_' + Date.now(),
        x: x,
        y: y,
      ...
      // Remove any background or unwanted properties that might cause the black box
      if (newShape.background) {
        delete newShape.background;
      }
      
      // Process the shape to fix rendering issues
      this.processShapeForRendering(newShape);
      
      console.log('Adding library shape to canvas:', newShape);
    ...
    /**
     * Process a shape to fix rendering issues like black outlines
     * @param {Object} shape - The shape to process
     */
    processShapeForRendering(shape) {
      // Fix stroke properties to remove black outline
      ...
      // Set stroke width to 0 to remove outlines
      shape.strokeWidth = 0;
      
      // Ensure the shape has proper fill properties
      if (!shape.fill) {
      ...
      if (shape.type === 'group' && shape.children && shape.children.length > 0) {
        // For group shapes, we need to process each child
        shape.children = shape.children.map(child => {
          // Process each child recursively
          this.processShapeForRendering(child);
          return child;
        });
      } else if (shape.type === 'path') {
        // For path shapes, ensure stroke properties are set correctly
        if (shape.strokeWidth === undefined) {
          shape.strokeWidth = 1;
        }
        if (!shape.stroke) {
          shape.stroke = shape.fill || 'transparent';
        }
      } else if (shape.type === 'svg') {
        // For SVG shapes, ensure proper rendering
        shape.stroke = 'transparent';
        shape.strokeWidth = 0;
      } else if (shape.type === 'image') {
        // For image shapes, ensure no border is shown
        shape.showBorder = false;
        shape.stroke = 'transparent';
        shape.strokeWidth = 0;
        
        // If the image has a background property, remove it
        if (shape.background) {
          delete shape.background;
        }
      }
